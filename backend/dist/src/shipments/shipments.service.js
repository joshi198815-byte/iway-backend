...
let ShipmentsService = class ShipmentsService {
    ...
    decorateOpportunity(shipment) {
        ...
        let marketScore = 38;
        if (shipment.status === client_1.ShipmentStatus.published)
            marketScore += 12;
        else
            marketScore += 6;
        if (offerCount === 0)
        ...
        if (weight > 0 && weight <= 15)
            ...
            marketScore += 4;
        if (declaredValue >= 50 && declaredValue <= 500)
            marketScore += 12;
        else
            marketScore += 6;
        ...
        if (hoursOpen <= 12)
            ...
            marketScore += 4;
        if (antiFraudScore > 70)
            marketScore -= 10;
        else if (antiFraudScore <= 25)
            marketScore += 4;
    ...
    constructor(prisma, geoService, commissionsService, notificationsService, storageService, realtimeGateway) {
        this.prisma = prisma;
    ...
    async create(payload) {
        const direction = this.geoService.resolveDirection(payload.originCountryCode, payload.destinationCountryCode);
        if (!direction) {
            throw new common_1.BadRequestException('Solo se permiten rutas Guatemala ↔ USA.');
        }
        const createdShipment = await this.prisma.shipment.create({
            data: {
                customerId: payload.customerId,
                status: client_1.ShipmentStatus.published,
                direction,
        ...
        await this.notificationsService.sendPushMany(matchingTravelers.map((traveler) => traveler.userId), 'Nuevo envío disponible', `Hay un nuevo envío ${createdShipment.id} en la ruta ${payload.originCountryCode.toUpperCase()} → ${payload.destinationCountryCode.toUpperCase()}.`, 'shipment_available', createdShipment.id);
    ...
    async findAvailableForTraveler(travelerId, role) {
        if (!['traveler', 'admin', 'support'].includes(role)) {
            throw new common_1.ForbiddenException('Solo viajeros o personal interno pueden ver oportunidades.');
        }
        if (['admin', 'support'].includes(role)) {
            const shipments = await this.prisma.shipment.findMany({
                where: {
                    status: { in: [client_1.ShipmentStatus.published, client_1.ShipmentStatus.offered] },
    ...
    async updateStatus(id, payload) {
        ...
        const updated = await this.prisma.shipment.update({
            ...
            data: {
                ...
                images: payload.status === client_1.ShipmentStatus.delivered && payload.imageUrls?.length
                    ? {
                        create: payload.imageUrls.map((imageUrl) => ({
                            imageUrl,
                            kind: 'delivery_proof',
        ...
        await this.prisma.shipmentEvent.create({
            data: {
                ...
                eventPayload: {
                    ...
                    deliveryProofCount: payload.status === client_1.ShipmentStatus.delivered ? payload.imageUrls?.length ?? 0 : 0,
                },
            },
        });
        ...
        if (payload.status === client_1.ShipmentStatus.delivered) {
            ...
            await this.notificationsService.sendPush(shipment.customerId, 'Envío entregado', `Tu envío ${shipment.id} fue marcado como entregado${(payload.imageUrls?.length ?? 0) > 0 ? ' con evidencia visual' : ''}.`, 'shipment_delivered', shipment.id);
            if (shipment.assignedTravelerId) {
                await this.notificationsService.sendPush(shipment.assignedTravelerId, 'Entrega registrada', `El envío ${shipment.id} quedó cerrado como entregado.`, 'delivery_closed', shipment.id);
            }
        }
